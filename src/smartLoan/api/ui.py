...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .btn-train { background: linear-gradient(135deg, #1e2524, #0f1816); border: 1px solid #2d4a3e; }
    .btn-train .icon { background: rgba(52,211,153,0.2); }
...
<body>
  ...
  <main>
    ...
    <div class="btn-row">
      ...
      <button class="action-btn btn-train" onclick="showPanel('train')">
        <div class="icon">⚙️</div>
        <span class="label">Train New Model</span>
        <span class="sub">Trigger full MLOps pipeline</span>
      </button>
    </div>
    ...
    <div class="panel" id="panel-train">
      <h2>⚙️ MLOps Pipeline — Train a new model</h2>
      <p style="font-size:0.85rem; color:#94a3b8; margin-bottom: 20px;">
        This triggers data ingestion, processing, SMOTE handling, parameter tuning, model training, and performance evaluation.
      </p>
      <div class="submit-row" style="margin-top:0;">
        <button class="submit-btn" id="trigger-train-btn" onclick="triggerTraining()" style="background: linear-gradient(135deg, #10b981, #059669);">🚀 Start Training Pipeline</button>
      </div>
      <div id="train-result" style="margin-top: 20px;"></div>
    </div>
  ...
  <script>
    ...
    const PRESETS = {
      ...
      medium: {
        limit:50000, sex:2, edu:2, mar:1, age:42,
        pay0:1, pay2:0, pay3:0, pay4:0, pay5:0, pay6:0,
        bill1:46000, bill2:45000, bill3:44000, bill4:43000, bill5:42000, bill6:41000,
        pamt1:1500, pamt2:1500, pamt3:1500, pamt4:1500, pamt5:1500, pamt6:1500,
      },
    };
    ...
    async function triggerTraining() {
      ...
      btn.disabled = true;
      resultEl.innerHTML = '<div class="loader"><div class="spinner"></div><span>Initializing training pipeline...</span></div>';
      ...
      try {
        ...
        if (data.status && data.status.running) {
          checkTrainingStatus();
        } else if (data.status && data.status.message && data.status.message.includes('already in progress')) {
          checkTrainingStatus();
        } else if (data.message && data.message.includes('started')) {
          checkTrainingStatus();
        } else {
          resultEl.innerHTML = '<div class="error-box">' + (data.message || JSON.stringify(data)) + '</div>';
          btn.disabled = false;
        }
      } catch (e) {
        resultEl.innerHTML = '<div class="error-box">Failed to start training: ' + e.message + '</div>';
        btn.disabled = false;
      }
    }

    let trainPollInterval;
    async function checkTrainingStatus() {
      ...
      trainPollInterval = setInterval(async () => {
        try {
          ...
          if (data.running) {
            resultEl.innerHTML = '<div class="loader"><div class="spinner"></div><span>' + data.message + '</span></div>';
          } else {
            clearInterval(trainPollInterval);
            btn.disabled = false;
            
            if (data.message && data.message.includes('failed')) {
              resultEl.innerHTML = '<div class="error-box">' + data.message + '</div>';
            } else {
              resultEl.innerHTML = '<div class="msg-box">✅ ' + data.message + '</div>';
            }
          }
        } catch (e) {
          clearInterval(trainPollInterval);
          btn.disabled = false;
          resultEl.innerHTML = '<div class="error-box">Lost connection to server: ' + e.message + '</div>';
        }
      }, 3000);
    }
